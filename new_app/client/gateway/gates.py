...
class Gates(threading.Thread):
    ...
    def open_connection(self, username: str, client_connection: socket.socket) -> str:
        private_client = ClientChatPrivate(
            host=self.local_private_chat_host, port=self.local_private_chat_port
        )
        ...
        listener_thread = threading.Thread(
            target=self.listen, args=(private_client.received_queue, client_connection)
        )
        listener_thread.start()

        self.private_users[username] = {"client": private_client, "is_diff": False}

        return "INTERNAL OK"

    def open_connection_external(
        self, username: str, host: str, port: int, client_connection: socket.socket
    ) -> str:
        ...
        self.private_users[username] = {"client": private_client, "is_diff": True}

        return "EXTERNAL OK"

    def open_connection_multi(
        self, username: str, host: str, port: int, client_connection: socket.socket
    ) -> str:
        private_client = ClientChatPrivate(host=host, port=port)
        private_client.start_chat()
    ...
    def send_message(self, username_from: str, username_to: str, msg: str) -> str:
        ...
        user_from.send_message(
            f"SENDPRIVATESINGLE {username_from} {username_from} {username_to} {msg}"
        )
        if user_to["is_diff"]:
            user_to["client"].send_message(
                f"CHAT_EKSTERNAL {username_from} {username_to} {msg}\r\n\r\n"
            )
        else:
            user_to["client"].send_message(
                f"SENDPRIVATESINGLE {username_to} {username_from} {username_to} {msg}"
            )

        return "Terkirim"

    def send_file(
        self, username_from: str, username_to: str, filename: str, content_file: str
    ):
        user_from = self.private_users[username_from]["client"]
        user_to = self.private_users[username_to]

        user_from.send_message(
            f"SENDFILESINGLE {username_from} {username_from} {username_to} {filename} {content_file}"
        )
        if user_to["is_diff"]:
            user_to["client"].send_message(
                f"FILE_EKSTERNAL {username_from} {username_to} {filename} {content_file}\r\n\r\n"
            )
        else:
            user_to["client"].send_message(
                f"SENDFILESINGLE {username_to} {username_from} {username_to} {filename} {content_file}"
            )
    ...
    def join_group(
        self, room_name: str, username: str, client_connection: socket.socket
    ) -> str:
        if room_name in self.group_chats:
            group_client = ClientChatGroup(
                host=self.local_group_chat_host, port=self.local_group_chat_port
            )
            group_client.start_chat()

            listener_thread = threading.Thread(
                target=self.listen,
                args=(group_client.received_queue, client_connection),
            )
            listener_thread.start()
        ...
        group_client_x = ClientChatGroup(
            host=self.local_group_chat_host, port=self.local_group_chat_port
        )
        group_client_x.start_chat()

        listener_thread = threading.Thread(
            target=self.listen, args=(group_client_x.received_queue, client_connection)
        )
        listener_thread.start()
        ...
        self.group_chats[room_name] = {
            ...
            "members": {username: {"client": group_client_x, "is_diff": False}},
        }

        return "Berhasil Join"

    def join_group_external(
        self,
        host: str,
        port: int,
        room_name: str,
        ...
        listener_thread = threading.Thread(
            target=self.listen, args=(group_client.received_queue, client_connection)
        )
        listener_thread.start()
        ...
        self.group_chats[room_name] = {
            ...
            "members": {username: {"client": group_client, "is_diff": True}},
        }
        ...
        for uname, data in self.group_chats[room_name]["members"].items():
            if data["is_diff"]:
                data["client"].send_message(
                    f"CHAT_GROUP_EKSTERNAL {username} {room_name} {room_name} {msg}\r\n\r\n"
                )
            elif uname == username:
    ...
    def send_file_group(
        self, username: str, room_name: str, filename: str, file_content: str
    ) -> str:
        for uname, data in self.group_chats[room_name]["members"].items():
            if data["is_diff"]:
                data["client"].send_message(
                    f"FILE_GRUP_EKSTERNAL {username} {room_name} {room_name} {filename} {file_content}\r\n\r\n"
                )
            elif uname == username:
                data["client"].send_message(
                    f"FILEGROUP {room_name} {username} {filename} {file_content}"
                )
                ...
                if message:
                    ...
                    if order == "OPENPRIVATE":
                        ...
                        result = self.open_connection(
                            username=username, client_connection=client
                        )
                        print(result)
                        ...
                        result = self.open_connection_external(
                            username=username,
                            host=host,
                            port=port,
                        ...
                        result = self.open_connection_multi(
                            username=username,
                            host=host,
                            port=port,
                            client_connection=client,
                        ...
                        result = self.send_file(
                            ...
                            username_to=username_to,
                            filename=filename,
                            content_file=content_file_string,
                        )
                        print(result)
                    ...
                    # ================== GROUP CHAT
                        ...
                        result = self.send_message_group(
                            username=username, room_name=room_name, msg=msg.getvalue()
                        )
                        print(result)
                        ...
                        result = self.send_file_group(
                            username=username,
                            room_name=room_name,
                            filename=filename,
                            file_content=content_file.getvalue(),
                        )
                        print(result)
